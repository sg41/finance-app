...
async def get_bank_token(bank_name: str) -> str:
    config = BANK_CONFIGS[bank_name] # Эта строка теперь безопасна, т.к. bank_name проверяется в эндпоинтах
...
async def fetch_accounts(bank_access_token: str, consent_id: str, bank_client_id: str, bank_name: str) -> dict:
    config = BANK_CONFIGS[bank_name] # Эта строка также безопасна
...
@app.get("/users/{user_id}/connections", summary="Получить список всех подключений пользователя")
...
@app.get("/users/{user_id}/connection/check/{bank_name}/{bank_client_id}", summary="Проверить наличие подключения в БД")
async def check_connection_exists(user_id: int, bank_name: str, bank_client_id: str, db: Session = Depends(get_db)):
    # --- НОВОЕ: Проверка, поддерживается ли банк ---
    ...
    if connection:
        return {"status": "exists", "connection_id": connection.id, "connection_status": connection.status}
    else:
        raise HTTPException(status_code=404, detail="Connection not found.")

@app.post("/users/{user_id}/connect/{bank_name}/{bank_client_id}", summary="Инициировать подключение")
async def initiate_connection(user_id: int, bank_name: str, bank_client_id: str, db: Session = Depends(get_db)):
    # --- НОВОЕ: Проверка, поддерживается ли банк ---
    if bank_name not in BANK_CONFIGS:
        raise HTTPException(status_code=404, detail=f"Bank '{bank_name}' not supported.")
    ...
    if existing_connection:
        return {"status": "already_initiated", "message": "Connection has been already initiated.", "connection_id": existing_connection.id}
    ...
    if response.status_code != 200:
        raise HTTPException(status_code=500, detail=f"Failed to create consent request: {response.text}")
...
@app.post("/users/{user_id}/check_consent/{connection_id}", summary="Проверить статус согласия")
async def check_consent_status(user_id: int, connection_id: int, db: Session = Depends(get_db)):
    ...
    if api_status == "authorized":
        ...
        try:
            ...
            if name and connection.full_name != name:
                connection.full_name = name; db.commit()
        ...
        if connection.status != api_status:
            connection.status = api_status; db.commit()
        return {"status": api_status, "message": f"Consent status is '{api_status}'. Please try again later."}

@app.delete("/users/{user_id}/connection/{connection_id}", summary="Удалить подключение")
